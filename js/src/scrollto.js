const scrollToButtons = document.querySelectorAll('.scrollToBtn');

scrollToButtons.forEach(scrollToButton => scrollToButton.addEventListener('click', scrollToF));
function scrollToF(e){
	e.preventDefault();
	const destination = e.target.getAttribute('href');
	const destinationNode = document.querySelector(destination);
	const destinationPosition = destinationNode.offsetTop;
